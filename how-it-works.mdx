...
1. **Launch.** A creator opens a machine only for a new coin created with the registered Meteora DBC configuration. The creator fixes the prize table and pays the seed.
...
The transfer hook is part of the source launch design while a coin is on DBC. It restricts curve transfers to the machine flow and is removed at graduation. Deployment status is tracked on [Status and addresses](/status-and-addresses).
