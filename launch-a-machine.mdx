...
- **Prize table:** up to eight rows, each with a multiplier and ticket count. It is immutable.
- **Extra seed:** optional tokens added to the mandatory seed. This is an irrevocable donation.
...
The program checks the registered DBC configuration, creator signer, fresh untraded DBC pool, Token-2022 mint rules, prize table, and seed limits. Existing coins and pools that already traded are refused. The transfer hook is part of the source launch design; deployment status is on [Status and addresses](/status-and-addresses).
