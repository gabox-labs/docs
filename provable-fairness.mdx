...
The creator fixes a prize table when the machine launches. Each row has a token multiplier and a number of **tickets**. The ticket counts must total 65,536. Your purchase stores the table, pack size and free vault inventory in its draw, so later purchases cannot change its result.
...
3. Recompute the tickets and amounts with the SDK's `settle(...)` function, or the formula above and [the math](/the-math). Compare each result and the total.
